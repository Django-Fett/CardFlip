(function() {
  ...
  var init = function() {
    ...
    // Add event delegation to have one event listener trap for ALL click events
    pageBody.onclick = clickHandler = function(e) {
      var debounce = false;
      var debouncedHideMatchedCards = function() {
	  ...
	  // memoize the above debounce flag
      clickHandler = function(e) {
        var _target, targetClasses;
        ...
        if (!debounce) {  // Restrict number of clicks
          ...
          if (
            ...
            if (clickedItems.length == 2) {
              setTimeout(debouncedHideMatchedCards, 800);
